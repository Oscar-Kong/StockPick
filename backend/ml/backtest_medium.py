"""Medium bucket backtest — uses strategy registry params."""
...
def run_medium_backtest(
    ...
    strategy = StrategyRegistry().get_active("medium")
    entry_fn, resolved_variant = get_entry_fn("medium", entry_variant)
    bt = strategy.backtest_params
    hold_days = int(hold_days_override if hold_days_override is not None else bt.get("hold_days", 20))
    stop_pct = float(stop_pct_override if stop_pct_override is not None else bt.get("stop_pct", 0.07))
    target_pct = target_pct_override if target_pct_override is not None else bt.get("target_pct", 0.10)
