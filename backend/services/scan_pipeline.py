"""Deep Scan pipeline — Stage A through persistence behind one interface."""
...
from services.scan_display import enrich_scan_display
...
from services.stage_a_ranking import rank_stage_a_candidates, select_stage_b_symbols
...
from services.scan_decomposition import DecomposedScanScore, build_decomposed_scores
...
def _apply_canonical_scores(
    result: StockResult, decomposed: DecomposedScanScore, *, stage_b_score: float
) -> StockResult:
    """Attach documented score fields at the persistence seam."""
    result.score = round(decomposed.ranking_score, 1)
    ...
    metrics["stage_b_score"] = round(stage_b_score, 2)
    metrics["ranking_score"] = round(decomposed.ranking_score, 2)
    metrics["alpha_score"] = decomposed.alpha_score
    metrics["confidence_score"] = decomposed.confidence_score
    metrics["tradability_score"] = decomposed.tradability_score
...
def _append_partial_data_fallback(
    *,
    ...
    hist = ctx.history
    fallback_score = compute_partial_data_fallback_score(hist)
...
def run_scan_pipeline(manager: "ScanService", job_id: str, options: ScanOptions | None = None) -> None:
    options = options or ScanOptions()
    from services.scan_manager import ScanJob  # runtime import avoids cycle
    ...
    screener = manager._get_screener(job.bucket)
            ...
            try:
                ...
                if not gate.passed:
                    record_scan_skip(
                        skipped_candidates,
                        symbol=symbol,
                        reason=gate.skip_reason or "rejected",
                        detail=gate.skip_detail,
                    )
                    if gate.skip_detail and gate.skip_reason != INVALID_PRICE:
                        logger.info("Excluded %s: %s", symbol, gate.skip_detail)
                    continue
                ...
                outcome = score_stage_b_candidate(
                    ...
                    quality_score=quality_score,
                    strategy_version=strategy.version_id,
                    quality_filter=gate.quality_filter or {},
                    scan_id=job.job_id,
                ...
                _apply_canonical_scores(result, decomposed, stage_b_score=score)
