...
`execution_enabled=false`. `provider_usage` exposes the last quote pass, deferred
symbols, provider request count, and daily budget usage. Each status includes state, quote freshness, price,
