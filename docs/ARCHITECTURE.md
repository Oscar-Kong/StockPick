...
| Penny stability assessment | `backend/scoring/penny_stability.py` | Deep module: point-in-time OHLCV + Alpha in; independent Stability, hard gates, daily Entry Risk proxy, and explainable metrics out. It does not alter Alpha; `rejected` Stability gates the candidate out before final penny ranking. Model version: `quant-v2-round2-stability-shadow-v1`. |
