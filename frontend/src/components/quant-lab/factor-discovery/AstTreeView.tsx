...
function AstNodeInner({ node, depth }: { node: Record<string, unknown>; depth: number }) {
  ...
  return (
    <li role="treeitem" aria-expanded={children?.length ? open : undefined} className="text-sm">
      <button
        type="button"
      ...
      {open && children?.length ? (
        <ul role="group" className="ml-4 border-l border-zinc-200 pl-2 dark:border-zinc-700">
...
function AstNode({ node, depth = 0 }: { node: unknown; depth?: number }) {
  ...
  if (typeof node !== "object") {
    return (
      <li className="font-mono text-xs text-zinc-600 dark:text-zinc-400" role="treeitem">
        {String(node)}
      </li>
    );
  }
...
export function AstTreeView({ ast, label = "Formula AST" }: { ast: unknown; label?: string }) {
  ...
  return (
    <div className="rounded border border-zinc-200 p-3 dark:border-zinc-700">
      ...
      <ul role="tree" aria-label={label} className="space-y-1">
        <AstNode node={ast} />
      </ul>
    </div>
  );
